...
import { LayoutDashboard, PlusCircle, Home, LogOut, User, AlertCircle, CheckCircle2, Minus, Plus, ShoppingBag, ClipboardList, PenTool } from 'lucide-react';
...
export default function PelayanPage() {
  ...
  useEffect(() => {
    // retrieve strictly logged in waiter info from localstorage
    if (typeof window !== 'undefined') {
      const storedId = localStorage.getItem('idPegawai') || localStorage.getItem('pegawai_id') || '';
      const storedName = localStorage.getItem('namaPegawai') || localStorage.getItem('pegawai_nama') || 'Unknown Waiter';
  ...
  useEffect(() => {
    if (view === 'dashboard' || view === 'new_order') {
  ...
  async function fetchTables(showLoading = true) {
    ...
    try {
      ...
      if (data.sukses) setListMeja(data.data);
    } catch (err) {
  ...
  async function fetchMenu() {
    try {
      ...
      const data = await res.json();
      if (data.sukses) setListMenu(data.data);
    } catch (err) {
  ...
  async function fetchActiveOrders(showLoading = true) {
    ...
    try {
      ...
      if (data.sukses) {
        ...
        const active = data.data.filter((p: Pesanan) => p.statusPesanan !== 'SELESAI' && p.statusPesanan !== 'DIBATALKAN');
  ...
  return (
    <div className="w-screen h-screen flex bg-[#00215e] font-sans overflow-hidden">
      ...
      <div className="w-[280px] bg-[#00215e] flex flex-col items-center py-10 shrink-0 border-r border-[#2c4e80]">
        ...
        >
          <LayoutDashboard className="w-5 h-5 mr-2" /> Tables
        </button>
        <button 
      ...
      <div className="flex-1 bg-[#2c4e80] p-10 rounded-tl-[40px] shadow-[inset_10px_10px_20px_rgba(0,0,0,0.4)] flex flex-col overflow-hidden">
         ...
         {view === 'dashboard' && (
           <div className="flex-1 flex flex-col overflow-hidden">
              <h3 className="text-white text-3xl font-extrabold tracking-wide mb-8 shrink-0">Restaurant Visual Table Map</h3>
              
              <div className="flex-1 overflow-y-auto [scrollbar-width:thin] pr-2 flex items-center justify-center bg-white/5 rounded-2xl border border-white/10 p-8 relative">
                {loading ? (
                   <p className="text-center text-white font-bold mt-10 text-xl">Loading floor plan...</p>
                ) : (
                  <div className="grid grid-cols-4 gap-x-12 gap-y-16 w-full max-w-4xl mx-auto">
                    {/* Visual Floor Plan Representation */}
                    {listMeja.map((meja) => {
                      const isAvailable = meja.status === 'TERSEDIA';
                      return (
                        <div key={meja.noMeja} className="flex flex-col items-center relative group">
                          {/* Chairs Representation */}
                          <div className="absolute -top-4 w-12 h-4 bg-gray-400 rounded-t-full opacity-50"></div>
                          <div className="absolute -bottom-4 w-12 h-4 bg-gray-400 rounded-b-full opacity-50"></div>
                          <div className="absolute -left-4 top-1/2 -translate-y-1/2 w-4 h-12 bg-gray-400 rounded-l-full opacity-50"></div>
                          <div className="absolute -right-4 top-1/2 -translate-y-1/2 w-4 h-12 bg-gray-400 rounded-r-full opacity-50"></div>
                          
                          {/* Table Body */}
                          <div className={`w-32 h-32 rounded-3xl flex flex-col items-center justify-center shadow-[0_10px_25px_rgba(0,0,0,0.5)] border-4 z-10 transition-transform group-hover:scale-105 cursor-pointer ${isAvailable ? 'bg-white border-[#588157]' : 'bg-[#1a2f4c] border-[#fc4100]'}`}
                               onClick={() => { setSelectedTable(meja.noMeja); setView('new_order'); }}>
                            <div className={`text-4xl font-extrabold mb-1 ${isAvailable ? 'text-[#00215e]' : 'text-gray-300'}`}>
                              {meja.noMeja}
                            </div>
                            <div className={`text-[10px] font-bold tracking-widest uppercase px-3 py-1 rounded-full ${isAvailable ? 'bg-[#588157]/20 text-[#588157]' : 'bg-[#fc4100]/20 text-[#fc4100]'}`}>
         ...
         {view === 'new_order' && (
           <div className="flex h-full space-x-6">
              ...
              <div className="flex-1 flex flex-col h-full overflow-hidden bg-white/5 p-6 rounded-2xl border border-white/10">
              ...
              <div className="w-[420px] bg-white rounded-2xl shadow-xl flex flex-col overflow-hidden shrink-0">
